...
int Grid::get_cell_index(const float p_x, const float p_y) const {
    const float diff_x = p_x - origin.x;
    const float diff_y = p_y - origin.y;
    const int i = diff_x / cell_size;
    const int j = diff_y / cell_size;
    ...
    for (int i = 0; i < particles.count; i++) {
        ...
        for (int o_y = -2; o_y <= 2; o_y++) {
            for (int o_x = -2; o_x <= 2; o_x++) {
                const float p_x_j = p_x_i + o_x * cell_size;
                const float p_y_j = p_y_i + o_y * cell_size;
                const int c_j = get_cell_index(p_x_j, p_y_j);
